...
if (Get-Command py -ErrorAction SilentlyContinue) {
    & py -3 "$rootDir/scripts/transifex/extract_strings.py"
    & py -3 "$rootDir/scripts/transifex/transifex.py" push --root "$rootDir"
} elseif (Get-Command python -ErrorAction SilentlyContinue) {
    & python "$rootDir/scripts/transifex/extract_strings.py"
    & python "$rootDir/scripts/transifex/transifex.py" push --root "$rootDir"
} elseif (Get-Command python3 -ErrorAction SilentlyContinue) {
    & python3 "$rootDir/scripts/transifex/extract_strings.py"
